...
object AkkaStreamTest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val program = Source.combine(
      ...
      .buffer(128, OverflowStrategy.backpressure)
      .async
      .groupedWithin(Integer.MAX_VALUE, FiniteDuration.apply(1, TimeUnit.SECONDS))
      .map(llc => ByteString(llc.size.toString+"\n"))

    val t1 = System.currentTimeMillis()
    val file = Paths.get(s"AkkaStreamTestRate(${num}-${time}).csv")
    val done = program.runWith(FileIO.toPath(file))
