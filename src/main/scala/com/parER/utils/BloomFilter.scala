...
/**
 *
 * Original package bloomfilter.mutable
 *
 */
@SerialVersionUID(2L)
class BloomFilter[T] private (val numberOfBits: Long, val numberOfHashes: Int, private val bits: UnsafeBitArray)
  ...
  def intersect(that: BloomFilter[T]): BloomFilter[T] = {
    ...
    val bf = new BloomFilter[T](this.numberOfBits, this.numberOfHashes, this.bits & that.bits)
    bf.bits
    bf
  }
  ...
  def cardinality() = bits.getBitCount
