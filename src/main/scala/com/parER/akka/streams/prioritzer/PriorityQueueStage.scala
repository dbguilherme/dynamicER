...
class PriorityQueueStage(name: String, opt: Long = 0) extends GraphStage[FlowShape[List[Comparison], List[Comparison]]] {
  val in = Inlet[List[Comparison]]("PriorityStage.in")
  val out = Outlet[List[Comparison]]("PriorityStage.out")
  ...
  override def createLogic(attr: Attributes): GraphStageLogic =
    new GraphStageLogic(shape) {
      ...
      val buffer = mutable.Queue[Comparison]()
