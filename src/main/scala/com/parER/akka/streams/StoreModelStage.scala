...
class StoreModelStage(size1: Int = 16, size2: Int = 16) extends GraphStage[FlowShape[Message, List[Comparison]]] {
  val in = Inlet[Message]("StoreModelStage.in")
  val out = Outlet[List[Comparison]]("StoreModelStage.out")
  ...
  override def createLogic(attr: Attributes): GraphStageLogic =
    new GraphStageLogic(shape) {
      ...
      setHandler(in, new InHandler {
        ...
        def solveComparisons(comparisons: List[Comparison]): Unit = {
          val comps = storeModel.solveComparisons(comparisons)
          if (comps.size > 0)
            push(out, comps)
          else
            pull(in)
