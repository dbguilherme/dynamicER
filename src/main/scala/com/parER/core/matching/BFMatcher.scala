...
class BFMatcher(val numberOfBits: Long, val numberOfHashes: Int) extends Matcher {
  ...
  val epsilon = 0.1 // TODO make it locally computable
  ...
  override def execute(c: Comparison): Comparison = {
    ...
    val malus = if (c_max > numberOfBits - epsilon) math.min(1.0 , (m_max.getSignatures.size() - numberOfBits + epsilon).toDouble / m_min.getSignatures.size())
    else 0
    val c_and : Double = bf1.intersect(bf2).cardinality()
