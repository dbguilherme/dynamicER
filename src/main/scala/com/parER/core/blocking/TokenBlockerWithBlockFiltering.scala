...
class TokenBlockerWithBlockFiltering(size1: Int, size2: Int = 0, ro: Double = 0.005, ff: Double = 0.8) extends TokenBlocker {
  ...
  def getBlocks(idx: Int, dId: Int, textModelTokens: List[String]) = {
    ...
    if (associatedBlocks.size > 0){
      val index = (associatedBlocks.size.toDouble * ff).ceil
