import SequentialFixedIncrementalCCMainNOBC.{tBlocker, tokenBlocker}
import SequentialFixedIncrementalDirtyMain.args
...
object SequentialFixedIncrementalDirtyMainNOBC extends App {
    ...
    def processEntity(i:Int, dIdx:Int, profile:EntityProfile)={
        ...
        val tuple = tokenBlocker.execute(id1, obj1) // HERE EXECUTE NOT PROCESS, ONLY FOR NOBC
...
//        tCompGeneration += (System.currentTimeMillis() - t)
    ...
    for ( inc <- 1 to N) {
        ...
        if (Config.output) {
            ...
            val line = List[String]("i-wnp-nobc", inc.toString, tEnd.toString, aggTime.toString, CCC, name, CoCl, ro, ff, PC, PQ, BBT, CCT, MAT, OT, ODT, BBC, CCC)
